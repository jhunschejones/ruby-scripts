...
input_file_duration = %x[ffprobe -i #{ARGF.argv[0]} -show_entries format=duration -v quiet -of csv='p=0'].chomp

if input_file_duration.to_f < 3.0
  %x[ffmpeg -y -hide_banner -loglevel panic -i #{ARGF.argv[0]} -af apad,atrim=0:3,loudnorm=I=-16:TP=-1.5,atrim=0:#{input_file_duration} -ar 44.1k #{ARGF.argv[1]}]
else
  %x[ffmpeg -y -hide_banner -loglevel panic -i #{ARGF.argv[0]} -af loudnorm=I=-16:TP=-1.5 -ar 44.1k #{ARGF.argv[0]}]
end
